        ...
        Nothing ->
            ...
            case state.currentBlock of
                ...
                Just block_ ->
                    let
                        block =
                            { block_ | body = AST.BlockUtilities.dropLast block_.body }
        ...
        Just rawLine ->
            ...
            case ( state.inBlock, Line.isEmpty currentLine, Line.isNonEmptyBlank currentLine ) of
                ...
                ( True, False, _ ) ->
                    let
                        match =
                            Maybe.map .heading state.currentBlock == inspectHeading currentLine
        ...
        Just block__ ->
            let
                block_ =
                    ...
                    in
                    block__
                        |> AST.BlockUtilities.updateMeta (\m -> { m | id = id })
                        |> AST.BlockUtilities.updateMeta (\m -> { m | numberOfLines = List.length block__.body })
                    ...
                    case block_.heading of
                        ...
                        Ordinary _ ->
                            case Dict.get "section-type" block_.properties of
                                Just "markdown" ->
                                    { block_ | body = block_.body |> AST.BlockUtilities.dropLast }
                                        |> finalize
                                        |> transformBlock
                        ...
                        Verbatim _ ->
                            if List.head block_.body == Just "```" then
                                ...
                                { block_ | body = AST.BlockUtilities.dropLast block_.body } |> finalize
            in
            { state
                | lines = List.drop 1 state.lines
                , lineNumber = state.lineNumber + 1
                ...
                , inBlock = False
                , inVerbatim = isVerbatimLine currentLine.content
                , currentBlock = Nothing
            }
