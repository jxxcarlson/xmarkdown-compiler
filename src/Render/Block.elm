...
renderBody count acc depth settings attrs block =
    case block.heading of
        Paragraph ->
            [ renderParagraphBody count depth settings attrs block ]
...
renderParagraphBody : Int -> Int -> RenderSettings -> List (Html.Attribute MarkupMsg) -> ExpressionBlock -> Html MarkupMsg
renderParagraphBody count depth settings attrs block =
    case block.body of
        Right exprs ->
            Html.p
